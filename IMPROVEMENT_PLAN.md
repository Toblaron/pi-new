...
All of Phase 6 (6a-6h) is done, including the era-detection fix and a full live functional smoke-test sweep. `resize_window`/real-device mobile screenshot testing is a confirmed, undocumented platform constraint of this environment (see top of doc) — not something further work here can resolve. Remaining, in priority order: **3b's style-controls extraction (dedicated session — by far the biggest remaining `Home.tsx` cluster, ~17-18 states) → 2b (OpenAPI spec drift closure, incremental/ongoing) → 4c (integration tests for history routes) → 2a (job-queue worker, only if something new needs it).**
Rationale: 6f is the only unfinished polish item and just needs the window-manager quirk resolved (or a real phone on the LAN) to close out properly — it currently rests on code-level inference, not observation. The era-detection bug is a real, verified inaccuracy but low urgency (cosmetic era tag, not a functional break). Style controls is deliberately last among the near-term items — it's pervasive enough (read by generation payload, batch, variations, presets, draft save/restore, artist memory) to deserve a full session of its own attention rather than being squeezed in.
