...
**Status at a glance**: Phases 0, 1a, 1b, 1c, 2b, 2c, 2d, 3a, 3c, 3d, 4a, 4b, 5, a critical bugfix pass, 6a-6h, and 7 (deep bughunt pass) are all DONE and deployed (commits through `212daa8`). Phase 3b is done for its agreed scope (4 of ~5 hooks extracted; style controls deferred). Remaining open work: Phase 2a (job queue — deliberately not built further, see Phase 7 and 6h below), 3b's style-controls extraction, and one deliberately-deferred low-severity cache-dedup finding from Phase 7 (see below).
...
**Known testing limitation, root-caused (2026-07-30)**: `resize_window` doesn't work, and it's not fixable from this environment. Installed `wmctrl`/`xdotool` to investigate directly: the actual Chrome window was maximized (`_NET_WM_STATE_MAXIMIZED_VERT/HORZ`), which explained why resize calls were silently reverted — un-maximizing let height changes apply, but **width hard-floors around 508px no matter what** (tried the MCP tool, tried direct `wmctrl -e` move-resize). This is Chrome's own internal minimum window width, not a window-manager setting — no WM-level fix exists. Separately, and more importantly: the page's own `window.innerWidth` never matched the actual OS window width at any point (stayed 937 regardless of the real window being 508-1920px), meaning the automation viewport is a fixed CDP-level override decoupled from real window geometry — a deeper layer than the width floor. **Conclusion: sub-500px mobile-viewport testing isn't achievable via window resize with this browser automation setup, period.** 6f's code-level evidence (grepped responsive Tailwind classes, found `grid-cols-2` with no mobile collapse) stands as the best available signal without a real phone on the LAN.
...
**Bug spotted along the way — fixed 2026-07-30**: `/suggest` returned `era: "2000s"` for a-ha's "Take On Me" (actually 1985). Root cause: `fetchMusicBrainzData()` only fetched `limit=5` recordings; MusicBrainz's default ordering is relevance, not date, so a heavily-re-released song can have all 5 top-relevance results be modern re-releases while the true original sits further down. Verified empirically: "Take On Me" has 95 total matching recordings, and every one of the top 5 by relevance was a 2000s-2010s reissue. Fix in `artifacts/api-server/src/lib/musicbrainz.ts`: bumped `limit` to 25, and decoupled `releaseYear` from the single duration-matched "best" recording — now computed as the earliest date across *all* fetched recordings' releases, since dating the song and picking the best genre/label/album match are different problems. Verified against the live MusicBrainz API directly and against the running `/api/suggest` endpoint: `era` now correctly returns `"80s"`.

### 6f. Mobile responsiveness — DONE for what's fixable without a real device
Live mobile-viewport screenshot testing is confirmed **not achievable** in this environment — see the root-cause investigation at the top of this doc (Chrome's own minimum window width, plus a CDP-level viewport override decoupled from real window geometry; not a bug, a platform constraint). Given that, fixed the concrete issue found via code inspection directly rather than leaving it unverified: `grid grid-cols-2` (no responsive override) on Vocals/Energy and Tempo/Era in `Home.tsx`, plus the same pattern in `TransitionBuilder.tsx` and `MultiTrackBuilder.tsx` — changed to `grid-cols-1 sm:grid-cols-2`, the standard Tailwind mobile-first idiom (deterministic CSS, trustworthy without a literal screenshot). Confirmed desktop rendering is unaffected. `SongDnaPanel.tsx`'s `grid-cols-2` left alone — already capped at `max-w-[200px]`, genuinely fine at any width. **Only a real phone on the LAN would add anything further here** — worth doing opportunistically, not worth blocking on.
...
### 6h. Functional smoke-test sweep — DONE (2026-07-30)

In response to "Lets get the app to an acceptable functional state," exercised every remaining feature that hadn't been explicitly clicked-through live this session, end-to-end in the real browser against the production instance, confirming each via a 200 response on its API call plus visual confirmation of correct rendered output (not just "no error"):

- **Remix Chain / Genre Pivot transform** (`POST /api/suno/transform`): clicked "Acoustic" on a live a-ha "Take On Me" result — version history advanced to 7 versions, breadcrumb showed `ORIGINAL > ACOUSTIC`, remix chain showed "1 step", quality score recomputed (99→94) with a new correctly-triggered warning about spare style-prompt characters.
- **Share links** (`POST /api/share`): confirmed 200 response; traced the handler (`handleShareTemplate` in `Home.tsx`) to confirm the actual UX is silent clipboard-copy + a 2.5s "Link copied!" button-label toast, not a modal — no bug, just documenting the real (non-obvious) behavior for future reference.
- **Reverse Suno** (`POST /api/reverse`): pasted a synthetic style-prompt fragment describing "Take On Me" without naming it; correctly inferred the source song at 95% confidence with accurate genre/mood/tempo/era/key tags.
- **Multi-Track Arrangement** (`POST /api/multi-track`): generated all 4 complementary tracks (Lead Vocal, Harmony/Backing, Instrumental Bed, Rhythm/Percussion) from a cached result; each rendered with working copy/download/export controls.
- **Transition Builder** (`POST /api/transition`): fed two different YouTube URLs (a-ha "Take On Me" + Ed Sheeran "Shape of You", the second uncached — exercised the full metadata+lyrics fetch path, not just cache reads); correctly identified both songs and produced a genre-blended transition template referencing both styles.

No new bugs found in this pass — every core feature works as designed. Combined with the era-detection fix (6e) and the mobile grid-collapse fix (6f), this closes out the "acceptable functional state" sweep: generation, all remix/transform/multi-track/transition/reverse features, sharing, history, and degraded-mode fallback have now all been verified live, not just by reading the code.

---

## Phase 7 — Deep bughunt pass — DONE (2026-07-30, commits `d84181a`, `212daa8`)

In response to "extensively bughunt the code, find critical errors, fill gaps, make sure everything is connected, responsive and working properly" — a more adversarial pass than 6h's functional smoke test. Three parallel audits (backend core generation pipeline, backend secondary routes/infra, full frontend), each instructed to report only findings verified by actually reading the code, not speculation. 37 findings came back; every one was independently re-verified by direct code reading before fixing (several were cross-confirmed by two audits independently, e.g. the rate-limiter IP-spoofing issue).

**Backend (14 of 15 findings fixed, full detail in commit `d84181a`)**: rate-limiter bypass via spoofable X-Forwarded-For (no `trust proxy` config existed at all), heavy rate-limit buckets silently colliding across routes (`req.path` is mount-relative), `/multi-track` and `/transition` never being rate-limited at the heavy tier despite 3-4x generation fan-out, unmatched `/api/*` returning 200 HTML instead of a JSON 404, the char-count validation failure path being genuinely silent, `dsp-measured` (the real-measurement tier) being mislabeled "estimated" in the AI prompt, a trust-boundary comment that didn't match the code next to it (client-supplied `feedbackContext` was still used as a fallback), unbounded-length prompt-injection surface via `genreNudge`/`excludeTags`, a DSP subprocess temp-file leak on timeout (Python doesn't run `with`-block cleanup on SIGTERM), share-link hash at only 32 bits with a misleading "or expired" message on a table that has no expiry, a missing index on a commonly-filtered column, and history mutations reporting false-positive success. One low-severity finding (no lock around cache population, causing wasted-but-harmless duplicate work under concurrent identical requests) was deliberately left unfixed — the audit itself scored it as no-correctness-impact, and a safe general-purpose fix would need to touch a 2400-line file's cache-stage plumbing in ways that risk introducing new concurrency bugs for a problem that doesn't currently cause any.

**Frontend (18 of 22 findings fixed, full detail in commit `212daa8`)**: the most serious cluster was rating/history state silently tracking the wrong entry — `rateCurrentTemplate` always assumed `history[0]` was on screen, which breaks the instant an older entry gets loaded while a newer, already-rated one sits at index 0; fixed by threading a `currentHistoryEntryId` through every path that displays a template (new generation, history load, variation merge, batch "Use Template"). Close behind: loading a track from a finished Batch run never synced the URL, so Share/Regenerate/MultiTrackBuilder would silently no-op or act on a stale song. Several stale-response races got proper guards (variation generation, batch retry, playlist-preview merge, analytics tag-stats fetch). Also fixed: creative-direction presets bleeding state into each other instead of resetting to baseline, Reverse Suno/Mood Board dropping the inferred tempo on apply, an array-index React key on an editable+removable list (could misattribute an uncommitted rename to the wrong section), a genre map silently truncating ~18 genre families with no "no results" feedback, and the complete absence of any ErrorBoundary in the app (added one, top-level only — true per-panel isolation would need one boundary per panel, judged out of scope for this pass). Four lower-value findings (a keyboard shortcut no-op in batch mode, a cosmetic selection-badge desync in the variation workshop, a version-control panel missing title-only diffs, and a merge path not passing `usedOptions`) were left as documented, lower-priority items rather than fixed, to keep the pass focused on things with real user-facing consequences.

Verified via: full monorepo typecheck (clean), all 126 backend vitest tests passing (plus one new regression test for the codepoint-trim fix), a production build, a live-server restart, and curl-based verification of the rate-limiter/404/share-hash fixes against the running instance (confirmed multi-track now actually rate-limits at request 21, confirmed generate-template/stream has its own separate budget from multi-track, confirmed the old 8-char share-hash format is now correctly rejected). Live browser verification of the frontend fixes was not possible this pass — the Claude-in-Chrome connection dropped mid-session and didn't reconnect — so those are typecheck/build-verified and carefully re-read, not click-tested. Worth a live pass through History rating, Batch "Use Template", and the creative-direction presets next time browser automation is available.
...
All of Phase 6 (6a-6h) and Phase 7 (deep bughunt pass) are done. `resize_window`/real-device mobile screenshot testing remains a confirmed, undocumented platform constraint of this environment (see top of doc) — not something further work here can resolve. Remaining, in priority order: **3b's style-controls extraction (dedicated session — by far the biggest remaining `Home.tsx` cluster, ~17-18 states) → a live browser pass through Phase 7's frontend fixes (History rating, Batch "Use Template", creative-direction presets — typecheck/build-verified but not click-tested, see Phase 7) → 2b (OpenAPI spec drift closure, incremental/ongoing) → 4c (integration tests for history routes) → 2a (job-queue worker — currently fully disconnected end-to-end, only worth building if something real needs it, not preemptively).**
Rationale: style controls is deliberately first among the near-term items now that the bughunt pass is done — it's pervasive enough (read by generation payload, batch, variations, presets, draft save/restore, artist memory) to deserve a full session of its own attention rather than being squeezed in. The Phase 7 live-verification pass is next since those fixes touch state correctness (ratings, history) that's easy to get subtly wrong and cheap to confirm with a few minutes of clicking once browser automation is available again.
