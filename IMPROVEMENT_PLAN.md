...
All of Phase 6a-6g except a real 6f re-verification, 3c, 3d, and 3b's agreed scope (4 hooks) are done. Remaining: **fix the resize_window issue and properly re-verify 6f with a real mobile screenshot → the era-detection bug spotted during 6e → 3b's style-controls extraction (dedicated session — by far the biggest remaining cluster) → 2b (incremental, ongoing) → 4c → 2a (only if something new needs it).**
