...
2. **Audio features** (permanent cache): BPM/key from description regex → GetSongBPM API → AI-knowledge fallback (`audioFeatures.ts`). **No real DSP analysis is performed** — confidence values reflect source quality, not measurement.
...
│   │           ├── acoustid.ts        # fpcalc/AcoustID fingerprinting — ⚠️ complete but NOT wired into any route
...
4. **API Spec Drift**: `openapi.yaml` only describes `/healthz` and `/generate-template`. When touching those, keep the spec in sync and run `pnpm --filter @workspace/api-spec run codegen`. Other endpoints use hand-written `fetch()` + local types in `Home.tsx` — keep both sides matching manually.
...
6. **Dormant subsystems**: `lib/acoustid.ts` (needs `fpcalc` binary + `ACOUSTID_API_KEY`) and `lib/jobQueue.ts` (no worker) are wired into nothing — don't assume they run.
