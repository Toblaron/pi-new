...
- **yt-dlp**: `lib/audioDownload.ts` shells out to the `yt-dlp` binary (must be on PATH) to fetch a short audio sample for AcoustID fingerprinting. `@distube/ytdl-core` can no longer extract playable stream URLs (its decipher logic is broken against YouTube's current player) and is only used elsewhere for metadata (`getInfo`/`getBasicInfo`), not byte downloads.
